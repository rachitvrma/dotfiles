...
(local edit_plugin_file
       (fn [filename]
         (string.format "<Cmd>edit %s/plugin/%s<CR>" (vim.fn.stdpath :config) filename)))
(local explore_at_file "<Cmd>lua MiniFiles.open(vim.api.nvim_buf_get_name(0))<CR>")
(local explore_quickfix
       (fn []
         (vim.cmd (if (not= (. (vim.fn.getqflist {:winid true}) :winid) 0)
                      "cclose"
                      "copen"))))
(local explore_locations
       (fn []
         (vim.cmd (if (not= (. (vim.fn.getloclist 0 {:winid true}) :winid) 0)
                      "lclose"
                      "lopen"))))
...
;; For the custom clipboard history picker, like yank ring in emacs
(nmap_leader :fy "<Cmd>Pick cliphist<CR>" "Clipboard history")
