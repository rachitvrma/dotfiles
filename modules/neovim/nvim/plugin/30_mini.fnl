  ...
  (fn []
    ((. (require :mini.basics) :setup)
     {;; Manage options in 'plugin/10_options.lua' for didactic purposes
      :options {:basic true} ;; Default: false
...
(now
  (fn []
    ...
    ((. (require :mini.icons) :setup)
     {:use_file_extension
      (fn [ext _]
        (not (or (. ext3_blocklist (ext:sub -3)) (. ext4_blocklist (ext:sub -4)))))})
...
(now (fn [] ((. (require :mini.notify) :setup))))
...
(now (fn [] ((. (require :mini.sessions) :setup))))
...
(now (fn [] ((. (require :mini.starter) :setup))))
...
(now (fn [] ((. (require :mini.statusline) :setup))))

;; Tabline. Sets `:h 'tabline'` to show all listed buffers in a line at the top.
;; Buffers are ordered as they were created. Navigate with `[b` and `]b`.
(now (fn [] ((. (require :mini.tabline) :setup))))
...
(now_if_args
  (fn []
    ...
    (local on_attach
           (fn [ev]
             (set (. vim.bo ev.buf :omnifunc) "v:lua.MiniCompletion.completefunc_lsp")))
...
(later (fn [] ((. (require :mini.animate) :setup))))
...
(later (fn [] ((. (require :mini.cmdline) :setup))))
...
(later (fn [] ((. (require :mini.input) :setup))))
...
(later (fn [] ((. (require :mini.jump2d) :setup))))
