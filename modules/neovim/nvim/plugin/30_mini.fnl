...
;; (now (fn [] (vim.cmd "colorscheme miniwinter")))

;; You can try these other 'mini.hues'-based color schemes (uncomment with `gcc`):
;; (now (fn [] (vim.cmd "colorscheme minispring")))
;; (now (fn [] (vim.cmd "colorscheme minisummer")))
;; (now (fn [] (vim.cmd "colorscheme miniautumn")))
;; (now (fn [] (vim.cmd "colorscheme randomhue")))
...
(now (fn []
       ((. (require :mini.basics) :setup) {;; Manage options in 'plugin/10_options.lua' for didactic purposes
                                           :options {:basic true}
                                           ;; Default: false
                                           :mappings {;; Create `<C-hjkl>` mappings for window navigation
                                                      :windows true
...
(now (fn []
       ...
       ((. (require :mini.icons) :setup) {:use_file_extension (fn [ext _]
                                                                (not (or (. ext3_blocklist
                                                                            (ext:sub -3))
                                                                         (. ext4_blocklist
                                                                            (ext:sub -4)))))})
               ...
               (local process_items
                      (fn [items base]
                        (MiniCompletion.default_process_items items base
                          process_items_opts)))
               ((. (require :mini.completion) :setup) {:lsp_completion {;; Without this config autocompletion is set up through `:h 'completefunc'`.
...
(now_if_args (fn []
               ...
               ;; NOTE: This is commented because it uses vim.pack.add() for itself
               ;; (local add_marks
               ;;        (fn []
               ...
               ;; NOTE: This is block that follows plugins installed using Home-Manager
               (local add_marks
                      (fn []
                        (MiniFiles.set_bookmark :c (vim.fn.stdpath :config)
                                                {:desc :Config})
                        (local vimpack_plugins
                               (.. (vim.fn.stdpath :data) :/site/pack/hm/start))
                        (MiniFiles.set_bookmark :p vimpack_plugins
                                                {:desc :Plugins})
                        (MiniFiles.set_bookmark :w vim.fn.getcwd
                                                {:desc "Working directory"})))
               (Config.new_autocmd :User :MiniFilesExplorerOpen add_marks
                                   "Add bookmarks")))
...
(later (fn []
         ...
         (ai.setup {;; 'mini.ai' can be extended with custom textobjects
                    :custom_textobjects {;; Make `aB` / `iB` act on around/inside whole *b*uffer
...
(later (fn []
         ...
         (miniclue.setup {;; Define which clues to show. By default shows only clues for custom mappings
                          ;; (uses `desc` field from the mapping; takes precedence over custom clue).
                          :clues [;; This is defined in 'plugin/20_keymaps.lua' with Leader group descriptions
...
(later (fn []
         ...
         (hipatterns.setup {:highlighters {;; Highlight a fixed set of common words. Will be highlighted in any place,
                                           ;; not like "only in comments".
                                           :fixme (hi_words [:FIXME
                                                             :Fixme
                                                             :fixme]
                                                            :MiniHipatternsFixme)
                                           :hack (hi_words [:HACK :Hack :hack]
                                                           :MiniHipatternsHack)
                                           :todo (hi_words [:TODO :Todo :todo]
                                                           :MiniHipatternsTodo)
                                           :note (hi_words [:NOTE :Note :note]
                                                           :MiniHipatternsNote)
...
(later (fn []
         ...
         (map.setup {;; Use Braille dots to encode text
         ...
         (each [_ key (ipairs [:n :N "*" "#"])]
           (local rhs ;; Also open enough folds when jumping to the next match
                  (.. key :zv
...
(later (fn []
         ...
         ;; Define a custom picker in the registry.
         ;; It then can be used like `:Pick cliphist` interactively or as
         ;; `'<Cmd>Pick cliphist<CR>'` right hand side in mappings.
         (set MiniPick.registry.cliphist
              (fn []
                (local choose
                       (fn [item]
                         (local decoded (vim.fn.system "cliphist decode" item))
                         (vim.fn.setreg "+" decoded)))
                (local source {:name :Cliphist : choose})
                (MiniPick.builtin.cli {:command [:cliphist :list]} {: source})))))
...
(later (fn []
         ...
         (snippets.setup {:snippets [;; Always load 'snippets/global.json' from config directory
                                     (snippets.gen_loader.from_file (.. config_path
